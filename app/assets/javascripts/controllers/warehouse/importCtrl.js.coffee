Sky.controller 'importCtrl', ['$routeParams', 'Common','Warehouse', 'TempImport', 'TempImportDetail'
($routeParams, Common, Warehouse, TempImport, TempImportDetail) ->
  ...
  @dataGridOptions =
    bindingOptions:
      dataSource: 'importCtrl.currentDetails'
    paging:
      enabled: false
    editing:
      editMode: 'row'
      ...
      removeEnabled: true
      insertEnabled: true
      removeConfirmMessage: "Bạn xác nhận muốn xóa dòng dữ liệu này?"
    selection:
      mode: 'multiple'
    columns: [
      {
  ...
  @DatepickerDemoCtrl = () =>
    ...
    @today()
    @clear = => @dt = null
    @disabled = (date, mode) =>return ( mode == 'day' && ( date.getDay() == 0 || date.getDay() == 6 ) )
    @toggleMin = => @minDate = @minDate ? null : new Date()
    @toggleMin()
